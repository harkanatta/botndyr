...
Rpakkar <- c("tidyverse", "diagram", "genogram", "xlsx", "vegan")
...
find_taxon_match <- function(x, kolgr_taxa, y) {
  ...
  # Iterate through each element in x (KolgrTaxa$Flokkun)
  for (i in 1:length(x)) {
    found_match <- FALSE
...
# Find the taxon match for each element in KolgrTaxa$Flokkun
...
merged_data_kolgr <- merge(KolgrTaxa, table_df, by.x = "matched_taxon_name", by.y = "Cleaned Taxon name")


write.csv(merged_data,"Feeding_Guilds/merged_data_allt.csv", row.names = F)
```
...
for (col in selected_cols) {
    ...
    # Create stacked bar plot for each year
    for (year in unique(merged_data$Artal)) {
        ...
        print(ggplot(plot_data, aes(x = factor(stod), y = value, fill = col)) +
    ...
    plot_title <- paste(LabX, year, "_", ".png", sep = "")
    ggsave(filename = plot_title, plot = p, width = 10, height = 7, dpi = 300)
    }
}
