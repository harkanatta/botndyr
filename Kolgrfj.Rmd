...
Rpakkar <- c("tidyverse","benthos","worrms", "plyr", "data.table", "DT", "dplyr", "here", "BBI", "ggh4x", "gplots","PDE", "magick", "tesseract", "xlsx", "ggpubr")
...
DF[DF$Flokkun %in% names(gagn)[1:10],] %>% ddply(.(Artal, stod,Flokkun),summarise, N=sum(N))%>% 
    filter(stod %in% c("C4", "A7", "B5", "B8", "E4", "E3")) %>% 
    mutate(Artal = factor(Artal)) %>% 
    ggplot(aes(x = Artal, y = N)) +
